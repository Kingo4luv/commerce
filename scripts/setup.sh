...
echo -n -e $(printf "${GREEN}Enter the environment you are setting up, followed by [ENTER]: ${ENDCOLOR}")
read env
...
ansible-playbook deploy/$environment/setup.yml --skip-tags "" -u $user -i deploy/$environment/inventory.ini --vault-id ~/.ansible/key --extra-vars 'ansible_become_pass="{{ sudo_password }}"' --extra-vars "branch=$branch"
